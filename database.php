...
function display($html = null, $content1 = null, $content2 = null, $content3 = null, $content4 = null, $content5 = null, $content6 = null, $content7 = null, $content8 = null, $content9 = null, $content10 = null) {
  if (isset($content1) && $content1 != "") {
    printf($html, $content1, $content2, $content3, $content4, $content5, $content6, $content7, $content8, $content9, $content10);
  }
}
...
function unurlify($text) {
  $text = urldecode($text);
  $text = str_replace("-", " ", $text);
  return ucwords($text);
}

function websiteify($url) {
  $url = trim($url, '/');
...
function getProfileById($id) {
  return DB::queryFirstRow("SELECT * FROM users WHERE id=%s", $id);
}

function get3Comments($id) {
  return DB::query("SELECT * FROM reviews WHERE startup=%s LIMIT 3", $id);
}

function getnComments($id) {
  $a = DB::queryFirstRow("SELECT count(*) as a FROM reviews WHERE startup=%s", $id);
  return $a["a"];
}

function get3News($id) {
  return DB::query("SELECT * FROM news WHERE startup=%s LIMIT 3", $id);
}

function getnNews($id) {
  $a = DB::queryFirstRow("SELECT count(*) as a FROM news WHERE startup=%s", $id);
  return $a["a"];
}

function numberify($n, $b) {
  if (($n - 5) > $b) {
    if (($n - $b) < 10) {
      return "5+";
    } else {
...
function get3Startups($id) {
  return DB::query("SELECT * FROM startups WHERE founder1 = %s OR founder2 = %s OR founder3 = %s OR founder4 = %s OR founder5 = %s LIMIT 3", $id, $id, $id, $id, $id);
}

function getnStartups($id) {
  $a = DB::queryFirstRow("SELECT COUNT(*) as a FROM startups WHERE founder1 = %s OR founder2 = %s OR founder3 = %s OR founder4 = %s OR founder5 = %s LIMIT 3", $id, $id, $id, $id, $id);
  return $a["a"];
}

function getStartupsById($id) {
  return DB::query("SELECT * FROM startups WHERE founder1 = %s OR founder2 = %s OR founder3 = %s OR founder4 = %s OR founder5 = %s", $id, $id, $id, $id, $id);
}

function getEmployeesRanges($choice) {
  switch ($choice) {
    case 0:
      return null;
      break;
...
function timeMe($t) {
  return gmdate("F d, Y", $t);
}

function socialMediaLink($link) {
  if ($link != null && $link != "") {
    if ($link[0] == "@") {
      $link = preg_replace('/(^|\s)@([a-z0-9_]+)/i', '$1https://twitter.com/$2', $link);
    }
